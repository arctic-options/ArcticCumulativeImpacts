...
setwd(file.path(dir_arctic,'HIACMS/CHI/stressors/ocean_acidification'))

# read in 1880-1889 data

data_nc_18 = nc_open('raw/cesm_co2sys_1880-1889.nc')
print(data_nc_18)

# read in 2005-2014 data

data_nc_20 = nc_open('raw/cesm_co2sys_2005-2014.nc')
print(data_nc_20)
...
annual_arag<-function(data,k){
  ...
  for(i in seq(1,120,by=12)){
    ...
    print(cellStats(out_laea,stat='mean',na.rm=T))
    
    #Saving rasters in LAEA projection for Arctic Options
    
    writeRaster(out_laea,paste0("working/global_arag_avg_laea",i,k, sep="_"),format='GTiff', overwrite=T)
...
writeRaster(decadal_arag_18,filename='working/global_oa_1880_1889_arag_mean',format='GTiff', overwrite=T)

#get the decadal average for 2005-2014

decadal_arag_20 = decadal_arag(arag_20)

writeRaster(decadal_arag_20,filename='working/global_oa_2005_2014_arag_mean',format='GTiff', overwrite=T)
...
diff = decadal_arag_20-decadal_arag_18


writeRaster(diff,filename='working/oa_arag_DecadalDifference.tif',overwrite=T)
