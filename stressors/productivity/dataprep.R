...
# for 2002-2003 and 2009-2013 and creating annual average rasters
...
setwd(file.path(dir_arctic,'HIACMS/CHI/stressors/productivity/raw'))
...
# I (jamie) added to this function to create an output raster rather than matrix of values
  ...
  vgpm.raster = function(file, w.lon, e.lon, n.lat, s.lat, log = TRUE, 
    ...
    #calc = log10(ROI/1000) #comment this out when just creating raw data
    calc = ROI/1000 #comment this out when doing log transform
    r = raster(calc)
    extent(r)<-e
    ...
    plot(r,col=color,
         ...
         ylab='Latitude',
         main=paste('Net Primary Production',strftime(day1,'%B %Y')),
         legend.lab=expression(paste('g C /', m^2,'/ day')),
         legend.mar=4.3)
    
    writeRaster(r,filename=paste0('../working/rasterized_rawdata/npp_raw',sep='_',strftime(day1,'%B %Y')),format='GTiff',overwrite=T) #use this for raw, non log transformed data
    #writeRaster(r,filename=paste0('../working/rasterized_rawdata/log/npp_log',sep='_',strftime(day1,'%B %Y')),format='GTiff',overwrite=T) #use this for log data
    
  }  # end of vgpm.raster() function
  
  
  
  files = list.files('vgpm.m.2008.xyz',full.names=TRUE)
